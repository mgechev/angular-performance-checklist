...
このドキュメントは、Angularアプリケーションのパフォーマンスを向上させるのに役立つ習慣の一覧です。
「Angular パフォーマンスチェックリスト」では、サーバーサイドでのプリレンダリングやアプリケーションのバンドル、実行時パフォーマンス、フレームワークによって実行される変更検出の最適化など、さまざまな話題について説明しています。
...
ほとんどの習慣はHTTP/1.1 と HTTP/2 の両方に有効です。
適用される可能性のあるプロトコルのバージョンを指定することによって、例外となる習慣についても言及します。
...
このセクションには、毎秒60フレーム（fps）のスムーズなユーザーエクスペリエンスを提供するために利用できる習慣が含まれています。
